...
let alice = { name = "Alice"; age = 19 }

let birthday p =
  p.age <- p.age + 1;
  p

let sum0 = ref (fun x -> 0)

let sum = function [] -> 0 | h :: t -> h + !sum0 t
...
(* AF: the float array [| x1; ...; xn |] represents the vector (x1, ..., xn)
 * RI: the array is non-empty *)
type vector = float array

(* [norm v] is the Euclidean norm of [v]. *)
let norm v = sqrt (Array.fold_left (fun acc x -> acc +. (x ** 2.)) 0. v)

let normalize_loop v =
  let euclidean_norm = norm v in
  for i = 0 to Array.length v - 1 do
    v.(i) <- v.(i) /. euclidean_norm
  done